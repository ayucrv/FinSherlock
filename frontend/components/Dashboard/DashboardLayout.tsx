...
interface Props {
  ...
  onSendMessage: (m: ChatMessage) => void
  onUpload: (file: File) => Promise<void>
  isUploading: boolean
}

export default function DashboardLayout({
  caseFiles, evidence, timeline, selectedCase, onSelectCase,
  chatMessages, onSendMessage, onUpload, isUploading,
}: Props) {
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex-1 overflow-y-auto p-6">
            <Workspace selectedCase={selectedCase} evidence={evidence} onUpload={onUpload} isUploading={isUploading} />
          </div>
        ...
        <div className="w-80 border-l border-gold-500/20 bg-noir-800/30 flex flex-col">
          <SherryChat messages={chatMessages} onSend={onSendMessage} reportId={selectedCase?.id} />
        </div>
      </div>
    </div>
  )
